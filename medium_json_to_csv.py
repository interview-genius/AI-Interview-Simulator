"""
Medium ingestion v3 -- Person B (Step 1, real fix for the 403s)

WHY v2 (requests + good headers) STILL FAILED:
Confirmed by testing from a real home IP with browser-like headers -- still
100% instant 403s. That rules out IP reputation and headers as the cause.
The real reason: Python's `requests` library (via urllib3/OpenSSL) produces a
TLS handshake fingerprint that's detectably different from a real browser's,
even when the HTTP headers on top look identical. Cloudflare-style bot
protection (which Medium uses) checks this at the network layer, before your
User-Agent string is ever read.

THE FIX: drive an actual Chromium browser with Playwright. The TLS handshake,
JS execution environment, and headers are then genuinely a browser's, because
it is one -- not a library imitating one.
...
Then run normally:
    python medium_full_scraper_v3.py
"""
