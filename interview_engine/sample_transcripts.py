"""
DEPRECATED -- these hand-written transcripts were stand-ins for Person A's
Step 6 mock-interview engine, which now exists as interview_engine/conversation.py
(ML Round mode).

feedback_scoring.py now prefers real transcripts generated by:
    python -m interview_engine.generate_test_transcripts

These samples are kept only as a fallback if no real transcripts exist yet.
They remain useful as adversarial edge cases (AMBIGUOUS_DELIVERY tests whether
the scorer decouples clarity from STAR/depth), but new tests should use the
real engine output.
"""
